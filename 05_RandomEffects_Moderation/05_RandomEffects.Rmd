...
Thus, when we pre- and post-multiply $\mathbf{G}_j$ by $\mathbf{Z}_j$ (transposing the second time), we will get an $n_j \times n_j$ matrix representing the contribution of random effects to the residual variance-covariance $\mathbf{V}_j$.
...
\tau_\textrm{int} & \tau_\textrm{int} & \tau_\textrm{int} & \tau_\textrm{int} & \sigma^2_e + \tau_\textrm{int}
\end{bmatrix} 
$$
...
More about this is provided here: https://quantdev.ssri.psu.edu/sites/qdev/files/ILD_Ch06_2017_MLMwithHeterogeneousVariance.html

# Cross-level interactions (incomplete -- in progress)

In analyses of experiments, we are often interested in whether a level 2 predictor (e.g., extraversion) influences a level 1 phenomenon. In the example above, we were curious to test whether a person's level of neuroticism (one measure per person, a level 2 predictor) moderated the within-person association of trial condition (incongruent vs. congruent) with reaction time on the flanker task. In this case, we represented between-person variation in the $cond \Leftrightarrow RT$ association as a random slope $\boldsymbol{\mu}_{1.} \sim \mathcal{N}(0, \tau_{11})$. And conceptually, neuroticism is a level 2 predictor of individual differences in this random slope, which is a latent variable at level 2.

This type of effect is called a 'cross-level interaction' because it reflects the interaction of an L2 variable with an L1 variable. If this is indeed an interaction, how does it play out in the MLM notation? Let's return to the case where we model RTs on the flanker as a function trial condition, allowing for a random intercept and random slope of condition per person.

## L1 equation

$$
RT_{ij} = \beta_{0j} + \beta_{1j}\textrm{cond}_{ij} + \varepsilon_{ij}
$$
...
## L1 predictor is random (i.e., random slope)

## L1 predictor is not random (i.e., fixed only)

## Nested random effects

[See this](vizrandomeffects.html)

## L2 predictors of

predictors of random slopes

## Latent disaggregation
